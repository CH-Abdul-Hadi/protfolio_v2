...
const PORTFOLIO_FILE_PATH =
  import.meta.env.VITE_PORTFOLIO_FILE_PATH || "/data/portfolio.json";
const DEFAULT_RESUME_PATH =
  import.meta.env.VITE_DEFAULT_RESUME_URL || "/resume/resume.pdf";
...
const defaultPortfolioData: PortfolioData = {
  personalInfo: {
    ...
    summary:
      import.meta.env.VITE_DEFAULT_BIO ||
      "A skilled developer passionate about creating impactful solutions.",
  ...
  socialLinks: {
    github:
      import.meta.env.VITE_DEFAULT_GITHUB_URL ||
      "https://github.com/lakshyajain-0291",
    linkedin:
      import.meta.env.VITE_DEFAULT_LINKEDIN_URL ||
...
export const portfolioStorage = {
  ...
  async getPortfolio(): Promise<PortfolioData> {
    try {
      ...
      try {
        ...
        console.log(
          "File data not available, checking localStorage:",
          fileError
        );
      }
  ...
  async saveToFile(data: PortfolioData): Promise<void> {
    try {
      ...
      const permission = await savedFileHandle.queryPermission({
        mode: "readwrite",
      });
      if (permission !== "granted") {
        const newPermission = await savedFileHandle.requestPermission({
          mode: "readwrite",
        });
    ...
    try {
      ...
      if (resumeFile.ok) {
        ...
        if (resumeBlob.type === "application/pdf") {
          ...
          console.log(
            "Resume file is not a PDF (received type:",
            resumeBlob.type,
            ")"
          );
