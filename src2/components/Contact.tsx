...
const Contact = () => {
  ...
  const githubUsername =
    (portfolio?.socialLinks?.github?.match(/github\.com\/([^\/]+)/) || [])[1] ||
    DEFAULT_SOCIAL.GITHUB_USERNAME;
  const linkedinUsername =
    (portfolio?.socialLinks?.linkedin?.match(/linkedin\.com\/in\/([^\/]+)/) ||
      [])[1] || DEFAULT_SOCIAL.LINKEDIN_USERNAME;
  const githubUrl = portfolio?.socialLinks?.github || DEFAULT_SOCIAL.GITHUB_URL;
  const linkedinUrl =
    portfolio?.socialLinks?.linkedin || DEFAULT_SOCIAL.LINKEDIN_URL;
  ...
  return (
    <section id="contact" className="py-12 sm:py-16 mb-3 lg:py-20">
